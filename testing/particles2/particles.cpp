...
int main() {
    ...
    // create a window
    GLFWwindow *window;
    if((window = glfwCreateWindow(width, height, "Particles Map Buffer", 0, 0)) == 0) {
    ...
    while(!glfwWindowShouldClose(window)) {
        ...
        for(int i = 0;i<particles;++i) {
            ...
            // euler iteration
            float r = ((float)rand()/(float)(RAND_MAX) - 0.5) * 3;
            velocity[i] += dt*g + glm::vec3(((float)rand()/(float)(RAND_MAX) - 0.5) * 3,
                                            ((float)rand()/(float)(RAND_MAX) - 0.5) * 2,
                                            ((float)rand()/(float)(RAND_MAX) - 0.5) * 2);
