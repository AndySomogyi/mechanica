...
class radius_mono {
	protected:
		...
		inline void r_init(int ijk,int s) {ignore(ijk); ignore(s);}
		/** Sets a required constant to be used when carrying out a
		 * plane bounds check. */
		inline void r_prime(double rv) {ignore(rv);}
		...
		inline double r_current_sub(double rs,int ijk,int q) {ignore(ijk); ignore(q); return rs;}
		...
		inline double r_scale(double rs,int ijk,int q) {ignore(ijk); ignore(q); return rs;}
